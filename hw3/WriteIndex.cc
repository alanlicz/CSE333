...
int WriteIndex(MemIndex* mi, DocTable* dt, const char* file_name) {
  ...
  // header.doctable_bytes = dt_bytes;
  // header.index_bytes = mi_bytes;
  // header.magic_number = kMagicNumber;
  // header.checksum = ;
  // header.ToDiskFormat();
  // fseek(f, 0, SEEK_SET);
  // fwrite(&header, sizeof(IndexFileHeader), 1, f);
  if (WriteHeader(f, dt_bytes, mi_bytes) == kFailedWrite) {
    fclose(f);
    unlink(file_name);
    return kFailedWrite;
  }
...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // Read the doctable into a buffer and feed it to the CRC.
  uint8_t curr_byte;
  for (int i = 0; i < doctable_bytes + memidx_bytes; i++) {
    if (fread(&curr_byte, sizeof(uint8_t), 1, f) != 1) {
      return kFailedWrite;
    }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    if ((record_bytes = WriteHTBucketRecord(
             f, record_pos, LinkedList_NumElements(ht->buckets[i]),
             bucket_pos)) == -1) {
      return kFailedWrite;
    }
    record_pos += record_bytes;

    // If the bucket is empty, we're done.
    if (LinkedList_NumElements(ht->buckets[i]) == 0) {
      continue;
    }

    int bucket_bytes;
    if ((bucket_bytes = WriteHTBucket(f, bucket_pos, ht->buckets[i], fn))
                                                                      == -1) {
      return kFailedWrite;
    }
...
static int WriteHTBucketRecord(FILE* f, IndexFileOffset_t offset,
  ...
  record.ToDiskFormat();
  // record.chain_num_elements = htonl(num_elts);
  // record.position = htonl(bucket_offset);
  ...
  // Otherwise, write the bucket contents.
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int32_t element_bytes =
        fn(f, element_pos, payload);  // number of bytes written
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = strlen(
      static_cast<const char*>(kv->value));  // you may want to change this
  ...
  return sizeof(DoctableElementHeader) +
         file_name_bytes;  // you may want to change this
}
...
static int WriteDocIDToPositionListFn(FILE* f, IndexFileOffset_t offset,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    // position = *static_cast<DocIDElementPosition*>(LLIterator_Get(it));
    ...
    // uint32_t position_truncated = static_cast<uint32_t>(position);
...
static int WriteWordToPostingsFn(FILE* f, IndexFileOffset_t offset,
  ...
  if (fwrite(wp->word, sizeof(char), word_bytes, f) != (size_t) word_bytes) {
    return kFailedWrite;
  }
  ...
  return sizeof(WordPostingsHeader) + word_bytes +
         ht_bytes;  // you may want to change this
}
}  // namespace hw3

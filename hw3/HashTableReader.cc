...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  // fseek moves the pointer to the correct position with offset
  ...
  for (int i=0; i<bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord curr;
    Verify333(fread(&curr, sizeof(ElementPositionRecord), 1, file_) == 1);
    curr.ToHostFormat();
    ret_val.push_back(curr.position);
  }

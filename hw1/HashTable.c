...
// The helper function takes 2 parameters: 1)the pointer to a hashtable
// and 2) the target key.
// It will return an iterator associate with the given key if the
// key already exists; and NULL otherwise.
LLIterator* InsertFindHelper(HashTable *table, HTKey_t key) {
  int bucket;
  LinkedList *chain;
  bucket = HashKeyToBucketNum(table, key);
  // find the chain associate with the key
...
bool HashTable_Insert(HashTable *table,
  ...
  LLIterator* li = InsertFindHelper(table, newkeyvalue.key);
  if (li == NULL) {
    // the key does not exist in the table
    HTKeyValue_t *kv = (HTKeyValue_t *)malloc(sizeof(HTKeyValue_t));
    Verify333(kv != NULL);
    *kv = newkeyvalue;
    LinkedList_Append(chain, kv);
    table->num_elements++;
    return false;
  }
  HTKeyValue_t *kv;
  LLIterator_Get(li, (LLPayload_t *)&kv);
  *oldkeyvalue = *kv;
  LLIterator_Free(li);
...
bool HashTable_Find(HashTable *table,
  ...
  LLIterator* li = InsertFindHelper(table, key);
  if (li == NULL) {
    return false;
  }
  LLPayload_t temp;
  LLIterator_Get(li, &temp);
  *keyvalue = *(HTKeyValue_t *)temp;
  LLIterator_Free(li);
  return true;
}
...
bool HashTable_Remove(HashTable *table,
  ...
  LLIterator_Get(li, (LLPayload_t *)&kv);
  *keyvalue = *kv;
  LLIterator_Remove(li, LLNoOpFree);
  LLIterator_Free(li);
  free(kv);
  table->num_elements--;
  return true;
...
bool HTIterator_IsValid(HTIterator *iter) {
  ...
  // check if the table is empty and if the current linked list
  // iterator is valid
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // check if the hashTable iterator is valid
  if (!HTIterator_IsValid(iter)) {
    return false;
  }

  // check if the current linked list iterator has next element
  ...
  while (iter->bucket_idx < iter->ht->num_buckets) {
    ...
    if (ht->buckets[iter->bucket_idx]->num_elements != 0) {
      LLIterator_Free(iter->bucket_it);
      iter->bucket_it =
...
bool HTIterator_Get(HTIterator *iter, HTKeyValue_t *keyvalue) {
  ...
  if (HTIterator_IsValid(iter)) {
    LLPayload_t temp;
    LLIterator_Get(iter->bucket_it, &temp);
    *keyvalue = *(HTKeyValue_t *)temp;

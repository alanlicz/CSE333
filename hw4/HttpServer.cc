...
static void HttpServer_ThrFn(ThreadPool::Task* t) {
  ...
  while (!done) {
    ...
    // Close the connection if the client sent a "Connection: close\r\n"
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  if (!query.empty()) {
    ...
    // Get matches.
    vector<hw3::QueryProcessor::QueryResult> matches =
    ...
    if (matches.size() == 0) {
      ...
      ret.AppendToBody("<p><br>\r\n");
      ret.AppendToBody(std::to_string(matches.size()) +
                       " results found for <b>");
      ret.AppendToBody(EscapeHtml(query));
      ret.AppendToBody("</b></p>\r\n");
      ret.AppendToBody("<ul>\r\n");
      for (const auto& match : matches) {
        if (match.document_name.compare(0, 5, "http:") == 0) {
          ret.AppendToBody("<li><a href=\"" + match.document_name + "\">" +
                           EscapeHtml(match.document_name) + "</a> [" +
                           std::to_string(match.rank) + "]" + "</li>\r\n");
        } else {
          ret.AppendToBody("<li><a href=\"/static/" + match.document_name +
                           "\">" + EscapeHtml(match.document_name) + "</a> [" +
                           std::to_string(match.rank) + "]" + "</li>\r\n");
        }
      }
  ...
  // Set protocol, response code, and message.
  ret.set_protocol("HTTP/1.1");
  ret.set_response_code(200);
  ret.set_message("OK");

...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char root_path_buffer[PATH_MAX];
  char test_file_buffer[PATH_MAX];

  // get the absolute paths
  if (realpath(root_dir.c_str(), root_path_buffer) == nullptr ||
      realpath(test_file.c_str(), test_file_buffer) == nullptr) {
    return false;
  }

  std::string root_path(root_path_buffer);
  std::string test_path(test_file_buffer);

  if (root_path.at(root_path.length() - 1) != '/') {
    root_path.append("/");
  }

  if (test_path.find(root_path) != 0) {
    return false;
  }
...
string EscapeHtml(const string& from) {
  ...
  // STEP 2
  size_t pos = 0;  // Position of the first character of the token
  while ((pos = ret.find_first_of("\"&'<>", pos)) != string::npos) {
    ...
    pos += 1;  // Skip the newly added escape code because maxisze is 5 (&apos
  }

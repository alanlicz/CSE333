...
static void GetPortAndPath(int argc,
  ...
  // STEP 1:
  // Check that there are at least 3 command line arguments
  if (argc < 4) {
  ...
  *port = atoi(argv[1]);
  ...
  // Set the path to the second argument
  *path = argv[2];
  ...
  // Add all remaining arguments as indices
  for (int i = 3; i < argc; i++) {
    string index = argv[i];

    // Check that the index is a readable file
    struct stat index_stat;
    if (stat(index.c_str(), &index_stat) != 0 || !S_ISREG(index_stat.st_mode)) {
      cerr << "Invalid index file: " << index << endl;
      Usage(argv[0]);
    }
  ...
  // Check that at least one index was provided
  if (indices->empty()) {
    cerr << "No index files provided" << endl;
    Usage(argv[0]);
  }
}

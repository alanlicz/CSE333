...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (stat(file_name, &file_stat) != 0) {  //
    // Error in fetching the file stats
    return NULL;
  }

  // Get the file size from the file stats
  *size = (int)file_stat.st_size;
  ...
  while (left_to_read > 0) {
    num_read = read(fd, temp_buf, left_to_read);
    if (num_read == -1) {
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  // The way I understand word_start is it's only NULL when we finish
  // processing a word.
  while (*cur_ptr != '\0') {
    if (isalpha(*cur_ptr) != 0) {
      ...
      *cur_ptr = tolower(*cur_ptr);
      ...
      // current character is a boundary between words
      if (word_start != NULL) {
        // end of a word
        // handle last word if it's not followed by a non-alphabetic character
        // e.g., "The quick brown fox jumps over the lazy dog"
        // no punctuations here
        *cur_ptr = '\0';  // overwrite boundary with null terminator
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    char *newstr;
    // Allocate memory for the WordPositions struct and initialize it.
    wp = (WordPositions*) malloc(sizeof(WordPositions));
    if (wp == NULL) {
      return;
    }
    newstr = (char*)malloc(sizeof(char) * strlen(word) + 1);
    if (newstr == NULL) {
      free(wp);
      return;
    }
    ...
    // Add the new position to the linked list of positions.
    LinkedList_Append(wp->positions, (LLPayload_t)(int64_t)pos);
    // Add the WordPositions struct to the hash table.
    kv.key = hash_key;
    kv.value = wp;

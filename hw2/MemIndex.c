...
// The number of buckets to use when allocating a new HashTable for a
// WordPostings' postings table.
#define POSTINGS_TABLE_INIT_BUCKETS 16
...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t doc_id,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    // HashTable resizes by itself so we start by a small value
...
LinkedList *MemIndex_Search(MemIndex *index, char *query[], int query_len) {
  ...
  // Iterate through the postings of the first query word and initialize the
  // search results
  ...
  for (i = 1; i < query_len; i++) {
    ...
    res = HashTable_Find(index, key, &kv);
    if (!res) {
      // If the query word is not found in the index

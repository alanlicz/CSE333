  ...
  for (i = 0 ; i < MAX_NUM_DIR_ENTRIES; i++) {
    ...
    if (stat(entries[i].path_name, &st) == 0) {
      ...
      if (S_ISREG(st.st_mode)) {
        ...
        // can find out using the S_ISDIR() macro. Again, look it up if you
        // don't know how to use it), then we'll need to recursively process i
...
static void HandleFile(char* file_path, DocTable** doc_table,
  ...
  // of the file.
  file_buf = ReadFileToString(file_path, &file_len);
  Verify333(file_buf != NULL);
